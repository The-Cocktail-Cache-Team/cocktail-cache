  ...
  .then(() => {
    ...
    allDrinks.forEach((item) => {
      for (let i = 1; i <= 15; i++) {
        ...
        if (
          item[objKey] &&
          !allDrinkIngredients.includes(item[objKey].toUpperCase())
        ) {
...
const displayCocktails = () => {
  ...
  if (allDrinksToDisplay.length > 0) {
    ...
    document.getElementById(
      "no-result-message"
    ).innerHTML = `SORRY, THERE ARE NO COCKTAILS IN OUR DATABASE THAT CONTAIN ${filters.join(
      " AND "
    )}. PLEASE TRY AGAIN.`;
...
function displayCurrentCocktail() {
  ...
  const cocktailIngredients = currentIngredients
    .map((item, index) => {
      if (ingredientMeasurments[index] !== null) {
        return `<button onclick="search('${item}')">${ingredientMeasurments[
          index
        ].toUpperCase()} ${item}</button>`;
      } else {
  ...
  document
    .querySelector(".current-new-image")
    .setAttribute("src", cocktailImgSrc);
  document
    .querySelector(".current-new-image")
    .setAttribute("alt", cocktailImgAlt);
...
function addFilterBtnEvntListener() {
  ...
  document
    ...
    .addEventListener("input", checkComplexityFilter);
  document
    .getElementById("complexity-slider")
    .addEventListener("change", updateDrinksToDisplay);

  document
    .getElementById("contains-alcohol")
    .addEventListener("change", updateDrinksToDisplay);
  document
    .getElementById("no-contains-alcohol")
    .addEventListener("change", updateDrinksToDisplay);
...
function populateFilterBox() {
  ...
  const allIngredients = allDrinkIngredients.map(
    (item) =>
      `<div class="ingredient-container" id="${item}" onclick="toggleFilter('${item}')"><p>${item}</p></div>`
  );
...
function updateDrinksToDisplay() {
    ...
    filters.forEach((ingredientName) => {
      const newArr = allDrinksToDisplay.filter((cocktailObj) => {
        for (let i = 1; i <= 15; i++) {
          ...
          if (
            cocktailObj[objKey] &&
            cocktailObj[objKey].toUpperCase() === ingredientName
          ) {
            return true;
          }
        }
  ...
  if (containsAlcoholRadio.checked == true) {
    let alcoholFilteredArray = allDrinksToDisplay.filter(
      (cocktailObj) => cocktailObj.strAlcoholic === "Alcoholic"
    );
...
const predictiveSearchContainer = document.getElementById(
...
const mainPredictiveSearchContainer = document.getElementById(
